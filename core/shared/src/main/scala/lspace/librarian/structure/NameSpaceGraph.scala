...
trait NameSpaceGraph extends DataGraph {
  ...
  trait Ontologies {
    protected[librarian] val byId: concurrent.Map[Long, Ontology] =
      new ConcurrentHashMap[Long, Ontology]().asScala
    protected[librarian] val byIri: concurrent.Map[String, Node] =
...
//                ontology.extendedClasses.foreach(_createEdge(node, Property.default.`@extends`, _))
  ...
  trait Properties {
    ...
    protected[librarian] val byIri: mutable.HashMap[String, Node] =
