...
//
//object PA extends OntologyDef(lspace.NS.vocab.Lspace + "librarian/P", label = "P", comment = "Predicate ontology") {
//
//  object keys
//
...
//    }
//  }
//  sealed trait CollectionHelper[T]
//      extends EqHelper[T]
//      with IntersectHelper[T]
//      with ContainsHelper[T]
//      with DisjointHelper[T]
//      with WithinHelper[T] {}
//
//  object SeqHelper {
...
//    }
//  }
//  sealed trait SeqHelper[T] extends CollectionHelper[T] with StartwithHelper[T] with EndswithHelper[T]
//
//  object DeepCollectionHelper {
//    def map[T <: P[T]](value: T): (T, DeepCollectionHelper[T]) = {
//      val helper: (_, DeepCollectionHelper[_]) = value match {
//        case v: List[P[_]] => v -> Helper.nestedPHelper[Any, P]
//        //        case v: Vector[T] => v -> Helper.vectorHelper[T]
//        case _ => throw new Exception("No SeqHelper found")
//      }
//      helper.asInstanceOf[(T, DeepCollectionHelper[T])]
//    }
//  }
//  sealed trait IntersectHelper[T] extends Helper[T] {
//    def intersect(avalue: Any, pvalue: T): Boolean
//  }
//  sealed trait ContainsHelper[T] extends Helper[T] {
//    def contains(avalue: Any, pvalue: T): Boolean
//  }
//  sealed trait DisjointHelper[T] extends Helper[T] {
//    def disjoint(avalue: Any, pvalue: T): Boolean
//  }
//  sealed trait WithinHelper[T] extends Helper[T] {
//    def within(avalue: Any, pvalue: T): Boolean
//  }
//  sealed trait StartwithHelper[T] extends Helper[T] {
//    def startsWith(avalue: Any, pvalue: T): Boolean
//  }
//  sealed trait EndswithHelper[T] extends Helper[T] {
//    def endsWith(avalue: Any, pvalue: T): Boolean
//  }
//  sealed trait DeepCollectionHelper[T <: P[_]]
//      extends IntersectHelper[List[T]]
//      with ContainsHelper[List[T]]
//      with DisjointHelper[List[T]]
//      with WithinHelper[List[T]]
//      with StartwithHelper[List[T]]
//      with EndswithHelper[List[T]]
//
//  object Helper {
...
//    }
//
//    implicit def nestedPHelper[T, F[Z] <: P[Z]] = new DeepCollectionHelper[F[T]] {
//      def intersect(avalue: Any, pvalue: List[F[T]]): Boolean = avalue match {
//        case avalue: Iterable[_] => pvalue.exists(p => avalue.exists(p.assert))
//        case _                   => false
//      }
//      def disjoint(avalue: Any, pvalue: List[F[T]]): Boolean = avalue match {
//        case avalue: Iterable[_] => !pvalue.exists(p => avalue.exists(p.assert))
//        case _                   => false
//      }
//      def contains(avalue: Any, pvalue: List[F[T]]): Boolean = avalue match {
//        case avalue: Iterable[_] => pvalue.forall(p => avalue.exists(p.assert))
//        case _                   => false
//      }
//      def within(avalue: Any, pvalue: List[F[T]]): Boolean = avalue match {
//        case avalue: Iterable[_] => avalue.exists(a => pvalue.exists(_.assert(a)))
//        case _                   => false
//      }
//      def startsWith(avalue: Any, pvalue: List[F[T]]): Boolean = avalue match {
//        case avalue: Iterable[_] =>
//          avalue.size >= pvalue.size && avalue.zip(pvalue).forall { case (v, p) => p.assert(v) }
//        case _ => false
//      }
//      def endsWith(avalue: Any, pvalue: List[F[T]]): Boolean = avalue match {
//        case avalue: Iterable[_] =>
//          avalue.size >= pvalue.size && avalue.toList.reverse.zip(pvalue.reverse).forall { case (v, p) => p.assert(v) }
//        case _ => false
//      }
//    }
...
//  def contains[T, P[Z] <: P[T]: ContainsHelper](value: P[T]): Contains[P[T]] =
//    p.Contains(value) //TODO ...
...
//        implicit ct: ClassTypeable.Aux[T0, T1, TT1]): p.And = p.And(_predicate :: predicate :: Nil)
//    def ||[T0, PR0[Z] <: P[Z], T1, TT1 <: ClassType[_]](predicate: PR0[T0])(
//        implicit ct: ClassTypeable.Aux[T0, T1, TT1]): p.Or = p.Or(_predicate :: predicate :: Nil)
//  }
//
...
//}
//
////trait PA[+T] extends Product with Serializable {
