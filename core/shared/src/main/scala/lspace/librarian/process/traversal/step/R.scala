...
object R extends StepDef("R") with StepWrapper[R] {
  ...
  object keys {
    private val resourceNode =
      MemGraphDefault.ns.upsertNode("sptth/tbd.tld/librarian/step/R/resource")
  ...
  def apply(values: List[Resource[Any]]): R = {
    ...
    values.foreach(v => node.addOut(keys.resource, node.graph.getDT(v).asInstanceOf[ClassType[Resource[Any]]], v))
