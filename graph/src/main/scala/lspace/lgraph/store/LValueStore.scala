...
class LValueStore[G <: LGraph](val iri: String, val graph: G) extends LStore[G] with ValueStore[G] {
    ...
    value match {
      case value: Int =>
        intCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.`@int`).asInstanceOf[Stream[T]])
      case value: Double =>
        doubleCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.`@double`).asInstanceOf[Stream[T]])
      case value: Long =>
        longCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.`@long`).asInstanceOf[Stream[T]])
      case value: String =>
        stringCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.`@string`).asInstanceOf[Stream[T]])
      case value: Boolean =>
        booleanCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.`@boolean`).asInstanceOf[Stream[T]])
      case value: Instant =>
        datetimeCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.`@datetime`).asInstanceOf[Stream[T]])
      case value: LocalDateTime =>
        localdatetimeCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.`@localdatetime`).asInstanceOf[Stream[T]])
      case value: LocalDate =>
        dateCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.`@date`).asInstanceOf[Stream[T]])
      case value: LocalTime =>
        timeCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.`@time`).asInstanceOf[Stream[T]])
      case value: Point =>
        geopointCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.`@geopoint`).asInstanceOf[Stream[T]])
      case value: Map[_, _] =>
        mapCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.mapType()).asInstanceOf[Stream[T]])
      case value: List[_] =>
        listCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.listType()).asInstanceOf[Stream[T]])
      case value: Vector[_] =>
        vectorCache
          .get(value)
          .map(_.toStream)
          .getOrElse(graph.storeManager.valueByValue(value, DataType.default.vectorType()).asInstanceOf[Stream[T]])
      case _ =>
  ...
  override def store(value: T): Unit = {
    ...
    graph.storeManager.storeValues(List(value))
  }

  override def store(values: List[T]): Unit = {
    values.foreach(super.store)
    graph.storeManager.storeValues(values)
  }
