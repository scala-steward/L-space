...
object Encode {
  ...
  implicit def toTextPlain[In](
      implicit e: EncodeText[In, Coeval],
      activeContext: ActiveContext,
      encoder: lspace.services.codecs.Decode[In, Text.Plain, String, Coeval]): io.finch.Encode.Aux[In, Text.Plain] = {
    io.finch.Encode
      .instance[In, Text.Plain]((a, cs) => Buf.ByteArray.Owned(e.encode(activeContext)(a).value().getBytes(cs.name)))
  }

  implicit def toTextHtml[In](
      implicit e: EncodeText[In, Coeval],
      activeContext: ActiveContext,
      encoder: lspace.services.codecs.Decode[In, Text.Html, String, Coeval]): io.finch.Encode.Aux[In, Text.Html] = {
    io.finch.Encode
      .instance[In, Text.Html]((a, cs) => Buf.ByteArray.Owned(e.encode(activeContext)(a).value().getBytes(cs.name)))
  }

  implicit def toApplicationJson[In](implicit e: EncodeJson[In, Coeval],
                                     activeContext: ActiveContext,
  ...
  implicit def toApplicationJsonLD[In](implicit e: EncodeJsonLD[In, Coeval],
                                       activeContext: ActiveContext,
  ...
  object streamEncoders extends StreamInstances {
    implicit def encodeJsonLDFs2Stream[In, F[_]: Effect](
        implicit
