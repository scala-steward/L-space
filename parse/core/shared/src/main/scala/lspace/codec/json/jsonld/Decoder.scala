...
//                .map(List(_))
      ...
      .flatMap { l =>
        Task.parSequenceUnordered(l).map { l =>
          resource
        }
      }
  }
    ...
    if (iri.isDefined && expandedJson.size == 1) {
      ...
      iri
        ...
        .flatMap { node =>
          expandedJson.extractOntologies.flatMap { ontologies =>
            for {
              _ <- Task.parSequence((ontologies ++ label.toList).map(node.addLabel))
              _ <- withEdges(node, expandedJson - types.`@context` - types.`@id` - types.`@ids` - types.`@type`)
            } yield node
          }
        }
...
//      case dt: TupleType[_] =>
    if (json.size != label.rangeTypes.size)
      ...
      Task
        .parSequence(json.zip(label.rangeTypes).map { case (json, types) => toScopedObject(json, types) })
        .map(_.map(_._2))
        .map {
          case List(a, b)                            => (a, b)
...
//                  - types.`@comment` - types.rdfsComment
            ...
            } yield Ontology.ontologies.getAndUpdate(node)) // .startAndForget
          } yield ontology
        })
      ...
      iri
        ...
        .map(_.flatMap { node =>
          ...
          for {
            ...
            property =
              try Property.properties.getAndUpdate(node)
              catch {
                case e: Exception =>
...
//                        println(s"removing $iri")
  ...
  def toProperties(json: Json)(implicit activeContext: ActiveContext): Task[List[Property]] =
    Task.parSequence(extractIris(json).map(toProperty))
...
//                  - types.`@comment` - types.rdfsComment
            ...
            } yield DataType.datatypes.getAndUpdate(node)) // .startAndForget
          } yield datatype
        })
        .getOrElse(Task.raiseError(FromJsonException(s"ontology without iri $expandedJson")))
    } else Task.raiseError(FromJsonException(s"ontology is not of type '@class' ${expandedJson.obj}"))
...
//    println(s"toClassType ${iri}")
  ...
  def parseOntology(jsons: List[Json], iri: String)(implicit activeContext: ActiveContext): Task[Ontology] =
    Task
      .parSequenceUnordered(
        jsons
          .map(_.obj)
        ...
        .map { obj =>
          obj.extractContext.flatMap { implicit activeContext =>
            ...
            if (expandedJson.contains(types.`@type`)) {
              ...
              expandedJson
                .get(types.`@graph`)
                .flatMap(json => json.list.map(parseOntology(_, iri)))
...
//              Task.raiseError(FromJsonException(s"cannot parse ontology, not @type or @graph ${expandedJson.keys}"))
        ...
        .orElse {
          json.list.map(parseOntology(_, iri))
        }
        .getOrElse(Task.raiseError(FromJsonException("Ontology resource is not an object")))
    }

  def parseProperty(jsons: List[Json], iri: String)(implicit activeContext: ActiveContext): Task[Property] =
    Task
      .parSequenceUnordered(
        jsons
          .map(_.obj)
          ...
          obj.extractContext.flatMap { implicit activeContext =>
            ...
            if (expandedJson.contains(types.`@type`)) {
              ...
              expandedJson
                .get(types.`@graph`)
                .flatMap { json =>
                  json.list.map(parseProperty(_, iri))
                }
                .getOrElse(Task.raiseError(FromJsonException("@graph is not an array")))
            } else {
