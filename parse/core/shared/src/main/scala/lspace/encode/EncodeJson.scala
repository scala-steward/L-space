...
object EncodeJson {
  ...
  private def _nodeToJsonMap[Json](
    resource: Resource[_]
  )(implicit encoder: JsonLDEncoder[Json], activeContext: ActiveContext): Json = {
  ...
  implicit def collectionToJson[T, CT <: ClassType[_], Json](implicit
    encoder: JsonLDEncoder[Json]
  ): EncodeJson[Collection[T, CT], Coeval] =
  ...
  protected[encode] def namedProjection[Json](projection: Projection, result: List[Any])(implicit
    encoder: JsonLDEncoder[Json],
    activeContext: ActiveContext
  ): Option[Json] = {
    import encoder._
  ...
  protected[encode] def _queryresultToJsonMap[Json](
