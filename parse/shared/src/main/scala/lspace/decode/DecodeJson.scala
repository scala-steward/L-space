...
object DecodeJson {
                  ...
                  obj.toList.flatMap {
                    case (key, value) =>
                      getProperty(key).map { key =>
                        decoder.toObject(value, key.range)(ActiveContext().asInstanceOf[decoder.AC]).map(key -> _)
                      }
                  }
                )
                .map { properties =>
  ...
  def bodyJsonTyped[T](label: Ontology, nodeToT: Node => T, allowedProperties: List[Property] = List())(
      implicit decoder: lspace.codec.Decode[Any, Any]): DecodeJson[T] = new DecodeJson[T] {
